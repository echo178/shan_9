...
interface Card{
    suit : number,
    honorCardRank : number,
    point : number,
   
}

interface PlayersCardPair{
    Card_1: Card,
    Card_2 : Card,
    rank ?: number,
}
interface PlayersThreeCardPair extends PlayersCardPair{
    Card_3 : Card,
}

class CardConstruct implements Card{
    suit : number
    honorCardRank: number
    point : number
    constructor(suitType :string,honorCard :string,point:number){
...
function evaluateSuitRank(suitType : string):number{
    switch(suitType){
        case('spade'):{
            return 4
        }
...
function evaluteHonorCardRank(honorCard:string):number{
    switch(honorCard){
        case('ace'):{
            return 4
        }   
...
function findHighestRankCard(CardPairArray : PlayersCardPair){
...
function card_1_higher_card_2_check(Card_1: Card,Card_2: Card){
    let status :boolean
...
function findHighestRankCard_Three(CardPairArray : PlayersThreeCardPair){
    function findHighestRankCard_compare( Card_1: Card,Card_2: Card){
        let highestCard : Card
...
function sortByPairRank(a : PlayersCardPair,b:PlayersCardPair){
    ...
    let A_highestCard : Card = findHighestRankCard(a)
    let B_highestCard : Card = findHighestRankCard(b)
...
function sortByPairRank_3cards(a : PlayersThreeCardPair,b:PlayersThreeCardPair){
...
function generateTotalCardPermutation(){
    ...
    let totalCombination = cardDeck.flatMap((obj1, i) => cardDeck.slice(i+1).map(obj2 =>{
        let returnObject : PlayersCardPair = {
            Card_1 : obj1,
            Card_2 : obj2
        }
        return returnObject
...
function removePairsWithKnownCard(totalPair : PlayersCardPair[],knownCardPair : PlayersCardPair[]) {
    
    let totalArray = totalPair;
    for (let i = 0; i < knownCardPair.length; i++) {
        
        totalArray = totalArray.filter(twoCardPair => {
            let currPair_Card1 : number[]= Object.values(twoCardPair.Card_1)
            let currPair_Card2 : number[] =  Object.values(twoCardPair.Card_2)
            let knownCardPair_1 : number[]= Object.values(knownCardPair[i].Card_1)
            let knownCardPair_2 : number[] = Object.values(knownCardPair[i].Card_2)
...
function checkTwoCardEqual(Card1 : number[], Card2 : number[]) {
...
function removeKnownCardsFromDeck(deck : Card[],ownCard : PlayersCardPair,knownCardArray : PlayersCardPair[]){
...
function searchCard_in_Combination(searchData : PlayersCardPair[] | PlayersThreeCardPair[],ownCardPair : PlayersCardPair){
...
function search_Pair_in_Combination(searchData : PlayersThreeCardPair[],ownCardPair : PlayersCardPair) {
    let Card_1 = ownCardPair.Card_1
    let Card_2 = ownCardPair.Card_2
...
function type_predicate_three_or_two_Cards(cardPair: PlayersCardPair | PlayersThreeCardPair): cardPair is PlayersThreeCardPair {
    return (cardPair as PlayersThreeCardPair).Card_3 !== undefined;
}
function getOwnCardStatus(ownCardPair : PlayersCardPair, knownCard : PlayersCardPair[], unknownPlayer : number = 1){
    ...
    let ThreeCardPairCombination : PlayersThreeCardPair[] = []
    ...
    for(let i = 0; i < PairRankWithout_8_9.length; i++){
        for(let j = 0 ; j< currentCardDeck.length; j++){
            if(!checkTwoCardEqual(Object.values(currentCardDeck[j]),Object.values(PairRankWithout_8_9[i].Card_1)) && !checkTwoCardEqual(Object.values(currentCardDeck[j]),Object.values(PairRankWithout_8_9[i].Card_2))){
                ThreeCardPairCombination.push({...PairRankWithout_8_9[i],Card_3: currentCardDeck[j]})
            }
    ...
    if( currentCard_beforeFilter !== undefined && currentCard_beforeFilter.Card_1 !== undefined &&  currentCard_beforeFilter.Card_2 !== undefined){
        ...
        let TwoCards_pointlimit_possiblePairRank_without_8_9 = possiblePairRank.filter((obj) =>{ 
            let totalpoint = obj.Card_1.point + obj.Card_2.point
            totalpoint = totalpoint % 10
            return checkTwoCardEqual(Object.values(currentCard_beforeFilter!.Card_1),Object.values(obj.Card_1)) && checkTwoCardEqual(Object.values(currentCard_beforeFilter!.Card_2),Object.values(obj.Card_2)) ||totalpoint > 3 && totalpoint !== 10 && totalpoint < 8}) 
        ...
        if(currentCard_afterFilter !== undefined && currentCard_afterFilter.rank !== undefined && currentCard_afterFilter_rank_in_pairRank_without_8_9 !== undefined && currentCard_afterFilter_rank_in_pairRank_without_8_9.rank !== undefined){
            ...
            return {
                ...
                possible_2cardpair_without_8_9 :TwoCards_pointlimit_possiblePairRank_without_8_9.length,
                rank_without_8_9 : currentCard_afterFilter_rank_in_pairRank_without_8_9.rank,
...
function createCardPair(suit_1: string, point_1 : string, suit_2: string, point_2: string){
...
function check_Card_Exist(cardPair : PlayersCardPair,checkCard : Card){
